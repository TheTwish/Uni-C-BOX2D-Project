...
Respawner::Respawner(b2World* world, const sf::Vector2f& position, const sf::Vector2f& size, const float& orientation, CollisionFilter type)
{
	b2BodyDef l_bodyDef;
	...
	case RESPAWNER:
		...
		l_fixtureDef.filter.categoryBits = RESPAWNER;
...
void Respawner::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
